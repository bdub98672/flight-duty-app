...
function isOff(value: string) {
  return (value || "").trim().toUpperCase() === "OFF";
}
...
function applyRules(row: DutyRow): DutyRow {
  const next = { ...row };
...
export default function Page() {
  ...
  const [lookbackRows, setLookbackRows] = useState<DutyRow[]>([]);
  ...
  async function loadData(pilot: string, year: number) {
    ...
    const yearRows = buildYearRows(year, pilot);
    ...
    setRows(mergedYearRows);
    setLookbackRows(priorRows);
  ...
  const derivedByDate = useMemo(() => {
    ...
    return map;
  }, [rows, lookbackRows, selectedYear]);
        ...
        <div className="rounded-2xl bg-white p-5 shadow">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600">Pilot, year, and month view</p>
            </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
            ...
            <span className="rounded-full bg-slate-100 px-3 py-1 text-slate-700">
              Duty times: type 0800, 2000, 930, or OFF
            </span>
